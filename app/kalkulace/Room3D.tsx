...
// Po najetí myší na stěnu se ukáže detail: rozměr, plocha a co se s ní bude dělat.
...
export default function Room3D({ walls, works = [], onSelectWall }: any) {
  ...
  return (
    <div className="relative">
      <svg viewBox={viewBox} className="mx-auto h-auto max-h-[440px] w-full" role="img" aria-label="3D náhled místnosti">
        ...
        {ordered.map((seg: any) => {
          ...
          return (
            ...
            >
              <title>Kliknutím přejdeš na zadání rozměrů stěny</title>
              <polygon
                points={pointsAttr(corners)}
                fill={active ? "var(--brand)" : "#e2e8f0"}
                fillOpacity={active ? 0.28 : 0.72}
                stroke={active ? "var(--brand)" : "#334155"}
              ...
              {wall.openings.map((opening: any) => {
                ...
                return <polygon key={opening.id} points={pointsAttr(quad)} fill={fill} fillOpacity="0.9" stroke={stroke} strokeWidth="1.5" pointerEvents="none" />;
              })}
      ...
      {/* Detail stěny po najetí myší */}
      {hovered ? (
        <div className="pointer-events-none absolute left-3 top-3 max-w-[260px] rounded-[var(--radius-sm)] border border-[var(--line)] bg-[var(--card)] p-3 text-xs shadow-lg">
        ...
        <div className="pointer-events-none absolute left-3 top-3 rounded-[var(--radius-sm)] border border-dashed border-[var(--line)] bg-[var(--card)]/80 px-2.5 py-1.5 text-[11px] text-[var(--muted)]">
      ...
      <div className="mt-2 flex flex-wrap items-center gap-3 text-[11px] text-[var(--muted)]">
        ...
        <span className="ml-auto">Stěny jdou po sobě doprava (1. dopředu, 2. doprava, …)</span>
      </div>
    </div>
  );
}
